...
export async function submitRegistration(data: OnboardingFormValues) {
  try {
    ...
    const otherNamesStr = otherAccounts
      .map((a) => a.name)
      .filter(Boolean)
      .join(", ");
    const otherUsersStr = otherAccounts
      .map((a) => a.user)
      .filter(Boolean)
      .join(", ");
    const otherPassesStr = otherAccounts
      .map((a) => a.pass)
      .filter(Boolean)
      .join(", ");

    const finalPlatforms = [...(data.delivery_platforms || [])].filter(
      (p) => p !== "Other" && p !== "None",
    );
    ...
    const formatHoursStr = (hours: any[]) => {
      ...
      return hours
        .map((h) => `${h.days?.join("/") || "N/A"} ${h.open}-${h.close}`)
        .join(" | ");
    };
    ...
    const payload: any = {
      ...
      location_addresses: data.location_addresses
        ?.map((la: any) => `${la.address} [Hours: ${formatHoursStr(la.hours)}]`)
        .join(", "),
      target_date: data.target_date
        ? new Date(data.target_date).toISOString().split("T")[0]
        : null,
    ...
    const response = await fetch(
      import.meta.env.VITE_N8N_URL + "/register-client",
      {
        method: "POST",
        body: submitData,
      },
